...
const handleSelectVocabulary = (vocabulary) => {
  const indexItem = _.findIndex(lessonObj.vocabularies, (item) => item?._id == vocabulary?._id)
  if (indexItem == -1) {
...
<template>
  ...
  <form method="post" class="form mt-4" autocomplete="off">
    <fieldset>
      <div class="grid grid-cols-12 gap-2">
        ...
        <div class="col-span-4 form-group">
          ...
          >
            ...
            <option v-for="progress in progresses" :key="progress?._id" :value="progress._id">
              {{ progress.name }}
            </option>
          </select>
        </div>
      ...
      <div class="mt-5">
        <RouterLink to="/admin/lesson" class="btn btn-outline-primary" style="margin-right: 4px"
          >Quay lại</RouterLink
        >
